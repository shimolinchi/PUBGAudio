...
def configuration(spec):
    ...
    if spec.get('split_protocol')=='known_assets_new_scenes':
        cfg.update(purpose='known client assets with independent synthetic scenes',display_version='v6')
...
def generate(job):
    ...
    root, cfg = STATE['root'], STATE['cfg']
    planner = plan.Planner(cfg, STATE['pools'][split], seed)
    density = planner.random_scene()
...
def main():
    ...
    print(json.dumps(dict(status='planned', scenes=len(jobs), seconds=len(jobs)*cfg['clip_seconds'],
    ...
    summary = dict(status='rendered_pending_verification', clips=len(jobs), seconds=len(jobs)*cfg['clip_seconds'],
