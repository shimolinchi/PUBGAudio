...
import vehicle_footstep_guard as vehicle_guard
...
def labels(scene,powers,cfg):
    ...
    if vehicle_guard.settings(cfg):
        vehicle_power, cabin_power = vehicle_guard.vehicle_powers(scene, powers)
        masked_by_vehicle = np.zeros(shape, np.uint8)
        for i,tr in enumerate(tracks):
            if tr['kind']=='footsteps' and not direct[i]:
                own_masked, loud_masked = vehicle_guard.interference(powers[i], vehicle_power, cabin_power, cfg)
                masked_by_vehicle[i] = own_masked | loud_masked
                visible[i] &= ~masked_by_vehicle[i].astype(bool)
        z.update(track_vehicle_masked=masked_by_vehicle, vehicle_power=vehicle_power.astype(np.float32))
...
def render(scene,library,spatial,cfg,path):
    ...
    if vehicle_guard.settings(cfg):
        # All vehicle sounds, including later arrivals and coasting tyres, are
        # evaluated before accepting any footstep actor. Admission is one draw
        # per whole actor, never repeated per step or based on actor order.
        count = (round(seconds*32000)-1024)//320+1
        vehicle_power = np.zeros(count); cabin_power = np.zeros(count)
        power_by_index = {}; kept = []
        order = [i for i,t in enumerate(relative['tracks']) if t['kind']=='vehicle']
        order += [i for i,t in enumerate(relative['tracks']) if t['kind']!='vehicle']
        for i in order:
            track = relative['tracks'][i]
            signal = render_track(track,library,spatial,seconds,cfg)*cfg['audio']['master_gain']
            track_power = m.frame_powers([signal],seconds)[0]
            if track['kind']=='vehicle':
                vehicle_power += track_power
                if track['source_role']=='self': cabin_power += track_power
            elif track['kind']=='footsteps' and track['source_role']=='external':
                keep, decision = vehicle_guard.keep_track(track,track_power,vehicle_power,cabin_power,cfg)
                if not any(d.get('decision')==decision['decision'] and d.get('track_id')==track['id']
                           for d in scene['decisions']):
                    scene['decisions'].append(decision)
                if not keep: continue
            mix += signal
            kept.append(i); power_by_index[i] = track_power
        kept.sort()
        scene['tracks'] = [scene['tracks'][i] for i in kept]
        powers = [power_by_index[i] for i in kept]
    else:
