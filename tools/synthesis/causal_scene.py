...
class Planner:
    ...
    def footsteps(self, start, role='external', forced_curve=None, end_seconds=None, fixed_gait=None):
        if self.cfg.get('body_actions'):
            from body_actions import footsteps
            return footsteps(self, start, role)
        c = self.cfg['footsteps']
        end = min(start+self.duration('footsteps') if end_seconds is None else end_seconds, self.t[-1]-1)
        groups = [g for g in self.groups if g.startswith('footsteps:') and
                  (fixed_gait is None or any(s['role']==fixed_gait for s in self.groups[g]))]
        ...
        if fixed_gait is not None:
            if fixed_gait not in weights:
                raise ValueError('Requested gait is unavailable on this surface')
            weights = {fixed_gait: 1.}
        ...
        for tick in np.arange(0,cfg['clip_seconds'],cfg['decision_step_seconds']):
            ...
            for kind,p in cfg['spawn_probability_per_eligible_step'][density].items():
                ...
                if kind not in ['aircraft','weather'] and not own:
                    own_probability = cfg['self_probability_when_eligible']
                    if kind=='footsteps':
                        own_probability = cfg.get('external_footsteps', {}).get('self_probability_when_eligible', own_probability)
                    role='self' if self.draw(own_probability,'direct_self',float(tick)) else 'external'
                foot_cfg = cfg.get('external_footsteps', {})
                if kind=='footsteps' and role=='external' and sum(
                        tr['kind']=='footsteps' and tr['source_role']=='external' for tr in busy
                        ) >= foot_cfg.get('maximum_simultaneous', 1000):
                    self.decisions.append(dict(time=float(tick),decision='external_foot_capacity',selected=False))
                    continue
                cabin=any(t['kind']=='vehicle' and t['source_role']=='self' for t in busy)
                if cabin and kind=='footsteps' and not foot_cfg.get('vehicle_guard') and not self.draw(cfg['masking']['cabin_foot_keep_probability'],'cabin_foot_admission',float(tick)):
                    continue
