...
由实际配置 `configs/generation_v5_1.json` 生成；完整声学证据与边界见 [衰减核查](attenuation_and_listener_v5_1.md)。上面的旧距离模式对脚步有如下专用覆盖。

配置 SHA-256：`57268891b8eb562c939a302a9ceeb61392d029d3f3b6ba20b710d5fb82c78920`
...
|---|---:|---:|---:|
| 脚步当前步态 | 3s | 12.5% | 12s |
| 车辆 accelerate | 1.5s | 16.6667% | 10s |
| 车辆 cruise | 1.5s | 25% | 10s |
| 车辆 decelerate | 1s | 20% | 10s |
| 车辆 coast_off | 2s | 12.5% | 10s |
...
删除当前状态及不合法后继后重新归一化。熄火滑行不能直接接巡航/动力减速，动力加速前先启动。急刹为 0.5–1.1s 过程，停留 1–3s；临近片末另有平滑刹停收尾，不作为随机巡航。
...
自然模式每段先停留：80%抽2–8s、20%抽10–25s；再选以下动作。初始朝向均匀0–360°、左右等权，smoothstep连接，角速度上限140°/s。小晃动65%概率向原朝向回摆40–100%，幅度和时间不同，避免机械周期。世界原点固定、俯仰0°。

| 段内动作 | 概率 | 幅度 | 持续时间 |
|---|---:|---|---|
| small_sway | 60% | [0.5, 6]° | [0.5, 2]s；必要时延长以满足角速度上限 |
| look_around | 25% | [8, 35]° | [1, 3]s；必要时延长以满足角速度上限 |
| large_turn | 15% | [60, 160]° | [0.8, 2]s；必要时延长以满足角速度上限 |
