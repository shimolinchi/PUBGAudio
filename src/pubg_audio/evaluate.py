...
class FrameDiagnostics:
    def __init__(self, threshold=.5, distance_scale=100.):
    ...
    def report(self):
        result = dict(threshold=self.threshold, frame_seconds=.1,
            ...
            localization_rule='detected single-source frames only; highest-confidence track, no truth-based selection',
...
def main():
    ...
    fingerprints = {split:hashlib.sha256((args.dataset/f'manifest_{split}.jsonl').read_bytes()).hexdigest()
    ...
    dataset = SyntheticScenes(args.dataset,args.split,classes=cfg.classes,distance_scale=scale,
    ...
    stats = FrameDiagnostics(args.threshold,scale)
    weighted = dict(total=0.,external=0.,self=0.)
    samples = 0
    with torch.inference_mode():
        for batch in DataLoader(dataset,batch_size=args.batch_size,shuffle=False,num_workers=args.workers):
            batch = {k:v.to(args.device) if isinstance(v,torch.Tensor) else v for k,v in batch.items()}
            outputs = model(batch['features'])
            loss, parts = training_loss(outputs,batch)
    ...
    report = dict(status='evaluated',scope='small synthetic held-out diagnostics, not official SELD scores',
        checkpoint_epoch=saved['epoch'],checkpoint_sha256=hashlib.sha256(args.checkpoint.read_bytes()).hexdigest(),
        split=args.split,scenes=len(dataset.rows),crops=samples,dataset_fingerprint=fingerprints,
        device=args.device,torch_version=str(torch.__version__),batch_size=args.batch_size,
