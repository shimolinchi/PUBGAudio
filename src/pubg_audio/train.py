...
def main():
    ...
    for epoch in range(start,start+epochs):
        ...
        record=dict(epoch=epoch,train_loss=train_loss,validation_loss=val_loss,train_batches=steps,
                    validation_batches=val_steps,smoke_only=args.smoke)
        ...
        if stale>=cfg.get('patience',75):
            break
    (args.output/'run.json').write_text(json.dumps(dict(status='smoke_passed' if args.smoke else 'training_completed',
        ...
        real_game_accuracy_not_evaluated=True),indent=2)+'\n',encoding='utf-8')
